...
### 0042. BOJ 2751 수 정렬하기 2. S5
:page_with_curl: https://www.acmicpc.net/problem/2751

```
N개의 수가 입력되었을 때, 오름차순으로 출력하는 문제.

단순 정렬 문제인 줄 알았으나, 문제 조건에 비해 정답률이 30%인것에 호기심을 가지고 왜 낮은지 확인하였음.

C++에서 라이브러리 형태로 제공하는 algorithm.h => sort()를 사용하면 간단히 해결 됨.

정답률이 낮은 이유는 제공 코드를 사용하지 않고 Quick sort를 직접 구현하는 문제였기 때문.

그렇기에, Quick Sort를 다시 구현해보는 기회가 되었음.

문제 해결은 algorithm.h 제공 정렬 함수를 사용하여 PASS. 

PASS 사유 : 

algorithm.h => sort() 의 경우 , quick sort 알고리즘을 바탕으로 최악의 경우에도 대비를 해놓은 sort.

어떠한 경우에도 O(n*logn)을 만족하는 함수임.
```